...
VERIFIER = "verify_phase_11_self_improvement_cycle"
VERIFIER_FILE = "projects/agifcore_master/05_testing/phase_11_governed_self_improvement/verify_phase_11_self_improvement_cycle.py"
REPORT_PATH = vc.EVIDENCE_DIR / "phase_11_self_improvement_cycle_report.json"
RUNTIME_IMPORTS = (
...
def build_pass_report() -> dict[str, object]:
    ...
    assert cycle.schema == "agifcore.phase_11.self_improvement_cycle.v1"
    assert cycle.overlay_contract.read_only_phase10_refs
    ...
    return vc.build_pass_report(
        ...
        assertions=[
            ...
            {"id": "phase10-refs-remain-read-only", "result": "pass"},
            {"id": "overlay-evidence-refs-present", "result": "pass"},
            {"id": "cycle-size-stays-bounded", "result": "pass"},
        ],
        anchors={"contradiction_case": cycle_dict},
        notes=["the cycle coordinator stays thin and keeps Phase 10 references read-only"],
    )
...
def main() -> int:
    ...
    if missing or not vc.runtime_modules_available(RUNTIME_IMPORTS):
        report = vc.build_blocked_report(
            ...
            blocker_message="self improvement cycle verifier could not import its runtime modules or found missing files",
            missing=missing,
        )
